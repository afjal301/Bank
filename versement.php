...
$error=null;
        if(isset($_GET["num"]) && $_GET["num"]!=="" && $_GET["montant"]!=="" && isset($_GET["montant"]) && isset($_GET["date"]) && $_GET["date"]!==""){
            ...
            $num_compte=$_GET["num"];
            $date_versement=$date_day;
            $montant_versement=$_GET["montant"];
    ...
    $query=$pdo->query("INSERT INTO mouvement(num_compte,date_mouvement,versement) VALUES('$num_compte','$date_versement','$montant_versement')");
               //mouvement_versement($_GET["num"],$date,$_GET["montant"]);
...
<body>
    <div class="versement">
        ...
        <form action="" class="from-group">
            ...
            <button type="reset" class="btn btn-danger"><a href="listevers.php" style="color:white;">Liste De Versement</a></button>
        </form>
